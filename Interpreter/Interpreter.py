...
def forecast_statement(identifier: str, column: str, model: str, params: dict, alias: str = None):
    ...
    if identifier.startswith('$'):
        ...
        if isinstance(variables[var_name], dict):
            if 'data' in variables[var_name]:
                ...
                return {"status": "error", "message": f"Variable ${var_name} doesn't contain valid data for forecasting."}
        elif hasattr(variables[var_name], 'columns'):
    ...
    if column not in df.columns:
        if len(df.columns) == 1:
            ...
            return {"status": "error", "message": f"Column '{column}' not found in dataset {identifier}. Available columns: {list(df.columns)}"}
    ...
    try:
        if model == "ARIMA":
            ...
            if len(data) < look_back + 1:
                return {"status": "error", "message": f"Not enough data for LSTM (need at least {look_back + 1} points)."}
    ...
    for statement in ast.get('statements', []):
        ...
        try:
            if stmt_type == 'Load':
                ...
                result = transform_statement(statement['table'], statement['column'], statement['interval']['amount'], alias)
                ...
                alias = statement.get('alias')
                result = forecast_statement(identifier, statement.get('column', 'value'), statement['model'], params, alias)
